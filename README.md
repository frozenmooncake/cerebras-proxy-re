...
2. **框架预设 (Framework Preset)**：选择 **Other**（纯 Python/API 项目无需选择前端预设）。
...
3. **各项设置**：
   ...
   * **安装命令**：建议填写 `pip install -r requirements.txt`（Vercel 通常会自动识别并运行）。

4. **配置环境变量**：点击展开 **Environment Variables**，将 `CEREBRAS_API_KEYS` 等所有必需密钥逐个添加。
...
6. *提示：为防止 Render 15 分钟无流量休眠，可用 UptimeRobot 每 10 分钟 Ping 一次 `https://your-app.onrender.com/health`。*
...
5. 在 **Environment Variables** 中设置 `CEREBRAS_API_KEYS` 等配置。

6. 点击 **Deploy** 即可完成公网上线。
